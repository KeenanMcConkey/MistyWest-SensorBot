...
class motor_intermediate:
    ...
    def write_vel(self):
		while not rospy.is_shutdown():
			if self.vel.angular.z == 0 or self.vel.angular.z > MINIUMUM_TURN: 
				self.vel_pub.publish(self.vel)
				self.vel_rate.sleep()
			else:
				self.minvel.angular.z = cmp(self.vel.angular.z,0)*MINIUMUM_TURN
				self.vel_pub.publish(self.minvel)
				time.sleep(abs(self.vel.angular.z)/(MINIUMUM_TURN)/VEL_PUBLISH_RATE)
				self.vel_pub.publish(self.zero_vel)
				time.sleep((1-abs(self.vel.angular.z)/(MINIUMUM_TURN))/VEL_PUBLISH_RATE)
				self.vel_rate.sleep()
    '''
...
if __name__ == '__main__':
    try:
		...
		while not rospy.is_shutdown():
            controller.write_vel(self)

    except rospy.ROSInterruptException:
        controller.set_vel(self, 0.0, 0.0)
